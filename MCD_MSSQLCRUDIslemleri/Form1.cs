...
namespace MCD_MSSQLCRUDIslemleri
{
    public partial class Form1 : Form
    {
        ...
        static string baglanti = "Server=DESKTOP-TUMHS1A\\GAMZEKURAL; Database=NORTHWND; Trusted_Connection=True;";
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    cmd = new SqlCommand("insert into Products (ProductName, UnitsInStock,UnitPrice) values(@ProductName, @UnitInStock,@UnitPrice)", conn);
                    cmd.Parameters.AddWithValue("@ProductName", txtAd.Text);
                    cmd.Parameters.AddWithValue("@UnitInStock", txtStok.Text);
